...
We simulate 15 time series of 25 periods each and compute the average interest rates. 
...
To produce the output from the simulations we then run the following code : 
...
The code gives us the following output, which we compare with the original output from the paper : 

![Table1](./tables/Table1.png)



## Case number 2 : $\varepsilon = \infty$ and $\mu$ is constant

In the case where $\varepsilon = \infty, we have a linear productivity function. 
Here, $\mu$ is still constant and equal to 1.0786. 

Similarly, we run a simualtion of 15 times series with 25 periods each. 
...
We run the preivous code to produce the tables. 

This simulation give us the following output, once again compared with the original output : 
...
## Case number 4 : $\varepsilon = \infty$ and $\mu$ is a function of $E[R_{t+1}]$

We run simulations, as before, for 15 time series and 25 period for each. Here, the production function is lenear and the risk $\mu$ is a function of the expected interest rate. 
...
save("results_sims.jld2", results_sims)
```
After running the tables file (which can also be found in case number 1), we get the following output :
