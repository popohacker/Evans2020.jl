...
function get_Ht(wt,args)
    ...
    default = false
    if tau === nothing 
        x = (wt .* n1 .+ x1 .- c_min .- K_min)[1] ### On ne sait pas trop s'il y a qu'un seul élément dans le vecteur
        Ht = min(Hbar, x)
		if Ht < Hbar
            default = true
        end
...
function get_ExpMU_c2tp1_k(k2tp1, zt, args)
    ...
    #(Exp_1pr_MU_CRRA_c2, _) = quadgk(Ex_args -> get_1pr_MU_c2_pdf(Ex_args), A_min, Inf)
    #(Exp_c2_1mgam, _) = quadgk(Ex_args -> get_c2tp1_1mgam_pdf(Ex_args), A_min, Inf)
...
function get_k2tp1(k2t, zt, args)
    ...
    elseif default == false
        ...
        elseif (k2tp1_max - K_min) >= krange_tol
            ...
            ###fx = ZeroProblem(eul_args -> get_Eul_err(eul_args), k2_init)
            ####results = solve(fx)
            results = Optim.optimize(k -> get_Eul_err(k, eul_args), [k2_init], BFGS())
            ...
            # k2tp1 = results.root
            k2tp1 = Optim.minimizer(results) #on doit obtenir les valeurs de x qui minimisent la fonction
            c1t = wt*n1 + x1 - k2tp1 - Ht
            Eul_err = get_Eul_err(Optim.minimum(results)) #on doit obtenir la valeur minimum
            MU_c1 = get_MUc_CRRA(c1t, 1.0)
...
function sim_timepath(
    ...
    while (t_ind < T) && !default
        ...
        if default ## MAYBE BECAUSE BOOLEAN TURNED INTO STRING
